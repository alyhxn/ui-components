...
async function program (opts) {
  ...
  let form_input_el = await form_input(subs[3], form_input_protocol)
  form_input_el.classList.add('hide')
  form_input_placeholder.replaceWith(form_input_el)
  ...
  function form_input_toggle_view(display) {
    toggle_view(form_input_el, display === 'block')
  }
  ...
  // ---- Protocols ----
  function form_input_protocol (send) {
    _.send_form_input = send
    return on
    function on ({ type, data }) { 
      console.log('form input data', type, data)
  ...
  function steps_wizard_protocol(send) {
    ...
    return on
    function on ({ type, data }) { 
      console.log('step wizard data', type, data)
      if (type === 'step_clicked') {
        ...
        _.send_form_input({ type: 'step_data', data })
      }
    }
  }

  async function render_form_component(component_name) {    
    let index = get_form_input_component_index(component_name)

    const component_fn = component_modules[component_name]
    if (!component_fn) throw new Error(`Unknown component "${component_name}"`)
      
    form_input_el = await component_fn(subs[index], form_input_protocol)
    form_input_placeholder.replaceWith(form_input_el)
  }
  ...
  function on_quick_actions_message ({ type, data }) {
    if (type == 'display_actions') {
      ...
      if (data === 'none') {
        ...
        form_input_toggle_view(data)
        cleanup()
      }
    } else if (type == 'action_submitted') {
      console.log(variables)
      is_completed = variables[data?.total_steps - 1]?.is_completed
      if (is_completed) {
        alert(JSON.stringify(variables.map(step => step.data), null, 2))
        if (_.send_quick_actions) {
          _.send_quick_actions({
            type: 'deactivate_input_field'
          })
        }
      }
    }
