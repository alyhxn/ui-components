...
async function graph_explorer (opts) {
  ...
  function build_and_render_view (focal_instance_path = null) {
    let top_element_path = null
    let top_element_offset = 0
    const first_visible = get_first_visible_element()
    if (first_visible) {
      top_element_path = first_visible.dataset.instance_path
      top_element_offset = first_visible.getBoundingClientRect().top - el.getBoundingClientRect().top
    }

    view = []
    build_view_recursive('/', '', 0, true, false, [])

    let focal_index = view.findIndex(node => node.instance_path === (focal_instance_path || top_element_path))
    if (focal_index === -1) focal_index = 0
    ...
    requestAnimationFrame(() => {
      const new_top_element = container.querySelector(`[data-instance_path="${top_element_path}"]`)
      if (new_top_element) {
        el.scrollTop = new_top_element.offsetTop - top_element_offset
      }
    })
  }
  ...
  function build_view_recursive (base_path, parent_instance_path, depth, is_last, is_hub, parent_pipe_trail) {
    ...
    const new_pipe_trail = [...parent_pipe_trail, !is_last]

    // 1. Expand hubs first (if expanded)
    if (state.expanded_hubs && entry.hubs) {
      entry.hubs.forEach((hub_path, i, arr) => {
        build_view_recursive(hub_path, instance_path, depth + 1, i === arr.length - 1, true, new_pipe_trail)
      })
    }

    // 2. Push the node itself
    view.push({ base_path, instance_path, depth, is_last, is_hub, pipe_trail: parent_pipe_trail })

    // 3. Expand subs (if expanded)
    if (state.expanded_subs && entry.subs) {
      entry.subs.forEach((sub_path, i, arr) => {
        build_view_recursive(sub_path, instance_path, depth + 1, i === arr.length - 1, false, new_pipe_trail)
      })
    }
  }
  ...
  function get_first_visible_element () {
    for (const child of container.children) {
      if (child.matches('.node') && child.getBoundingClientRect().bottom > el.getBoundingClientRect().top) {
        return child
      }
    }
    return null
